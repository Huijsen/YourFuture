...
app.get('/chats/:pageName/messages', (req, res) => {
  const { pageName } = req.params;
  const messages = chats[pageName] || [];
  res.json(messages);
});

// POST a new message
app.post('/chats/:pageName/messages', (req, res) => {
  const { pageName } = req.params;
  ...
  if (!chats[pageName]) chats[pageName] = [];
  chats[pageName].push(newMessage);
