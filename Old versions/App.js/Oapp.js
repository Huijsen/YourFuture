...
export default function App() {
  ...
  */}
  ...
  const [lastRunTime, setLastRunTime] = useState(0); // timestamp of last update
  ...
  const handleRunAlgorithm = async () => {
    ...
    try {
      ...
      // use your user's ID dynamically
      const myId = mlUser.id;              // e.g., "1"
      const myRankings = data.best_to_worst[myId]; // get your ranking list
              ...
              <View style={{ padding: 16 }}>
                ...
                {myRanking.map((u, i) => (
                  <Text key={i} style={{ marginBottom: 4 }}>
                    User {u.mac} — Similarity: {(u.score * 100).toFixed(1)}%
                  </Text>
                ))}
              </View>
            )}
